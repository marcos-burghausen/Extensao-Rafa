...
export const getConfigProperties = (): { [config: string]: unknown } => {
  return extensions.getExtension('rafaelburghausen.rafa-theme')?.packageJSON
...
export const getThemeConfig = (section: string) => {
  return getConfig('rafa-theme').inspect(section);
};
...
export const setThemeConfig = (
  ...
  return getConfig('rafa-theme').update(section, value, global);
};
...
export const isThemeActivated = (global: boolean = false): boolean => {
  return global
    ? getConfig().inspect('workbench.iconTheme')?.globalValue === 'rafa-theme'
...
const getExtensionPath = () =>
  extensions.getExtension('rafaelburghausen.rafa-theme')?.extensionPath ?? '';
